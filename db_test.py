...
connection_string = 'postgresql://<имя_пользователя>:<пароль>@<адрес_хоста>:<порт>/<имя_базы_данных>'
...
class Reflink(Base):
    ...
    def get_original_link(cls, link_id):
        ...
        try:
            link = session.query(Reflink).filter(Reflink.link_id == link_id).first()
            if link:
...
class Users(Base):
    ...
    def add_user(cls, user_id, user_name, user_lang, registry_datetime):
        ...
        session.commit()
        logging.info("User succesfully saved!")
...
async def shorten_url(url):
    ...
    short_url = hash_digest[:8]     # Используем первые 8 символов хеша в качестве сокращенного идентификатора
    return short_url
